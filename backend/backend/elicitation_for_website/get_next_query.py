...
def add_rec_dual_variables(
        ...
        if problem_type == "maximin":
            for k in range(K):
                if responses[k] == -1 or responses[k] == 1:
                    m.addConstr(alpha_vars[k] + mu_var <= 0, name=f"alpha_constr_k{k}_resp{responses[k]}")
    ...
    for f in range(num_features):
        lhs_1 = quicksum(
            [responses[k] * z_vectors[k][f] * alpha_vars[k] for k in range(K) if (responses[k] == 1 or responses[k] == -1)]
        ) + quicksum(
                (rho_vars[k] - nu_vars[k])
                * z_vectors[k][f]
                for k in range(K) if (responses[k] == 0)
            )
