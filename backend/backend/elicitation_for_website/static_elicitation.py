...
def add_r_constraints(
    ...
    for f in range(num_features):
        ...
        if len(fixed_queries) > 0:
            lhs_1_fixed = quicksum(
                ...
                * (
                ...
                for i_q, k in enumerate(K_fixed) if (response_scenario[k] == 1 or response_scenario[k] == -1)
            ) + quicksum(
                ...
                * (
                ...
                for i_q, k in enumerate(K_fixed) if (response_scenario[k] == 0)
            )
        ...
        lhs_1_free = quicksum(
            ...
            * quicksum(
                ...
                for k in K_free if (response_scenario[k] == 1 or response_scenario[k] == -1)
            )
            for i in range(num_items)
        ) + quicksum(
            items[i].features[f]
            * quicksum(
                ...
                for k in K_free if (response_scenario[k] == 0)
            )
            for i in range(num_items)
        ) - quicksum(
            items[i].features[f]
    ...
    return alpha_vars, rho_vars, nu_vars, beta_vars, v_bar_vars, w_bar_vars, v_bar_prime_vars, w_bar_prime_vars,\
           v_bar_prime_prime_vars, w_bar_prime_prime_vars
...
def feasibility_subproblem(z_vec_list,
    ...
    for i_item, item in enumerate(items):
        ...
        #partworth
        m.addConstr(quicksum(u_vars[i_item, i_feat] for i_feat in range(num_features)) == 1, name="sum_1")
